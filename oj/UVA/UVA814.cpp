...
int main()
{
    ...
    while(cin >> s && s != "*"){
        cin>>s>>k;
        while(k--) { cin>>t; addr.insert(t+"@"+s); }    //处理MTA,化为t@s
    }

    while(cin>>s&&s!="*"){
        parse_address(s, user1, mta1);        //user1,mta1指的是发送人

        vector<string> mta;               //收件人mta
        map<string, vector<string> >dest;   //key为mta，value为name
        set<string> vis;                    //收件人的名字
        while(cin>>t && t!="*"){
            parse_address(t, user2, mta2);   //收件人
