...
char name[maxn][maxl];
int score[maxn][5];
int removed[maxn];  //removed为1表示被删除

const char* course_name[] = {"Chinese", "Mathematics", "English", "Programming"};

int valid (int k)
{
    for (int i = 0; i < k; i++) if (!removed[i])  //没被删除
        {
            if (strcmp (sid[i], sid[k]) == 0) return 0;  //sid相同
        }
    return 1;
}
...
void add()   //添加记录
{
    for (;;)
    {
        ...
        if (valid (n))
        {
            score[n][4] = score[n][0] + score[n][1] + score[n][2] + score[n][3];  //总分
            n++;
        }
...
int rank (int k)
{
    int r = 0;
    for (int i = 0; i < n; i++)
        if (!removed[i] && score[i][4] > score[k][4]) r++;   //根据总分排名
    return r + 1;
}
...
double get_course_stat (int c, int s, int* passed, int* failed)    //判断某科挂科
{
    int tot = 0;
    *passed = *failed = 0;
    for (int i = 0; i < n; i++)
...
void get_overall_stat (int c, int* cnt)        //某学生挂科数
{
