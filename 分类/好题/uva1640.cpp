...
int f(int d, int n) {
  ...
  for(int i = 1; i < len; i++) {
    if(i == 1) ans++; // single digit
      ...
      ans += 9 * cnt[i-1];  // leading with another digit  n**  不包括0**
      if(d > 0) ans += pow10[i-1]; // leading with digit d  dxx
    }
  }
  ...
  for(int i = 0; i < len; i++) {
    ...
    int maxd = s[i] - '0' - 1;
    int mind = 0;
    if(i == 0 && len > 1) mind = 1; // no leading zeros allowed  即0***；但可以n0**
    for(int digit = mind; digit <= maxd; digit++) {
      ans += cnt[len-i-1];                          //n***
      if(i > 0) ans += pre[i-1] * pow10[len-i-1];   //dd***
      if(digit == d) ans += pow10[len-i-1];         //nd***
    }
  }
  return ans;
}
